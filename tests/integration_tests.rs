...
mod tests {
    ...
    fn test_spawn_help() {
        ...
        cmd.assert()
            .success()
            .stdout(predicate::str::contains(
                "Creates a new site with Nginx configuration",
            ))
    ...
    fn test_invalid_flag() {
        let mut cmd = cmd!();
        cmd.arg("spawn").arg(TEST_DOMAIN).arg("--invalid-flag");
        cmd.assert()
            .failure()
    ...
    fn test_deactivate_command() {
        let mut cmd = cmd!();
        cmd.arg("deactivate").arg(TEST_DOMAIN);
        cmd.assert()
            .failure()
            .stderr(predicate::str::contains("unrecognized subcommand"));
    }
    ...
    fn test_activate_command() {
        let mut cmd = cmd!();
        cmd.arg("activate").arg(TEST_DOMAIN);
        cmd.assert()
            .failure()
            .stderr(predicate::str::contains("unrecognized subcommand"));
    }
